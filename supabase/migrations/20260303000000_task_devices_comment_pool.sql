-- Layer 4: add comment_pool to task_devices.config from task.payload.comments (AI pre-generated).
-- App layer does not insert task_devices; trigger only. comment_pool = payload.comments array.
...
BEGIN
  ...
  -- Layer 4: pre-generated comments from Edge Function (payload.comments) → comment_pool in config
  IF _payload ? 'comments' AND jsonb_typeof(_payload->'comments') = 'array' THEN
    _cfg := _cfg || jsonb_build_object('comment_pool', _payload->'comments');
  END IF;
...
COMMENT ON FUNCTION public.fn_create_task_devices_on_task_insert() IS
  'Layer 3: after tasks INSERT, create task_devices for devices on non-busy PCs only. Layer 4: config.comment_pool from payload.comments (AI).';
