-- Add action_touch_coords to task_devices.config when created by trigger (from task.payload).
-- Agent reads cfg.action_touch_coords in task-executor.
...
BEGIN
  ...
  _cfg := jsonb_build_object(
    ...
    'comment_content', _payload->'comment_content',
    'action_touch_coords', _payload->'action_touch_coords'
  );
...
COMMENT ON FUNCTION public.fn_create_task_devices_on_task_insert() IS
  'Layer 3: after tasks INSERT, create task_devices for devices on non-busy PCs only (PC별 1개). config includes action_touch_coords from payload.';
