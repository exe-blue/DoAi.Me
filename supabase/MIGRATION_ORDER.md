...
**처음 한 번만:** 로그인 후 프로젝트 링크가 필요합니다.

```bash
# 1) Supabase 로그인 (브라우저에서 토큰 발급)
npx supabase login

# 2) 이 프로젝트를 원격 프로젝트에 링크 (DB 비밀번호 입력 프롬프트 나옴)
npm run db:link
...
이후에는 `npm run db:push`만 실행하면 됩니다. `supabase/migrations/` 폴더의 파일명 순서대로 적용되며, 이미 적용된 마이그레이션은 건너뜁니다.

- `supabase: command not found` → `npx supabase` 또는 `npm run db:push` 사용
- `Cannot find project ref` → 위 1~2단계(login → link)를 먼저 실행
- **원격 서버에서 `npm run db:push`가 "Missing script"일 때:** 최신 코드를 pull한 뒤 `npm run db:push`를 다시 시도하세요. 루트 `package.json`에 `db:push` 스크립트가 있어야 합니다. 그래도 없으면 `npx supabase db push`를 직접 실행하면 됩니다.
