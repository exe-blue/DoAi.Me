...
**YouTube 디바이스 플로우 (작은 단위 권장):**
1. **검색** — 영상 키워드(없으면 제목)로 검색 후 첫 영상 선택.
2. **시청** — 최소시청시간%~최대시청시간% 구간에서 랜덤 결정한 시간만큼 시청.
3. **대기** — 각 단계 후 `min_wait_ms`~`max_wait_ms`(ms) 랜덤 대기.
4. **액션** — 확률에 따라 좋아요/댓글(OpenAI 생성)/담기 실행.

`youtube/` 폴더 모듈(`search.js`, `watch.js`, `action.js`)을 우선 사용해 작은 단위로 실행하는 것을 권장(유지보수 용이). 동시 실행 기기 수는 기본 20대(`maxConcurrentTasks`).
...
| `setup/comment-generator.js` | **OpenAI API**로 댓글 생성. `OPENAI_API_KEY`(env 또는 agent-settings) 사용. `task-executor`가 시청 중 댓글 액션 시 호출. |
