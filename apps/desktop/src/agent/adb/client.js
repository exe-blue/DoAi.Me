/**
 * agent/adb/client.js — xiaowei.adb Device 래퍼 (Xiaowei WebSocket 경유)
 * API 규약: docs/xiaowei_ws_api
 *
 * 모든 명령은 Xiaowei WebSocket xiaowei.adb → ADB shell로 실행됨.
...
const { isXiaoweiFailure } = require('../core/xiaowei-constants');
...
class ADBDevice {
  ...
  constructor(xiaowei, serial, options = {}) {
    ...
    this.log = getLogger(`xiaowei.adb.${serial.substring(0, 8)}`);
  }
  ...
  /**
   * xiaowei.adb 명령 실행 (full adb prefix)
   * @param {string} command
   * @returns {Promise<string>}
   */
  async xiaoweiAdb(command) {
    this._ensureConnected();
    try {
      const res = await this.xiaowei.xiaoweiAdb(this.serial, command);
      return extractOutput(res, this.serial);
    } catch (err) {
      throw new ADBError(`xiaowei.adb failed: ${err.message}`, { serial: this.serial });
    }
  }

  /** 파일 push (PC → 디바이스) */
  async pushFile(localPath, remotePath) {
    const out = await this.xiaoweiAdb(`push "${localPath}" "${remotePath}"`);
    this.log.info('file_pushed', { local: localPath, remote: remotePath });
    return out;
  }

  /** 파일 pull (디바이스 → PC), Xiaowei WebSocket pullFile (docs/xiaowei_ws_api §4.3) */
  async pullFile(remotePath, localPath) {
    this._ensureConnected();
    const res = await this.xiaowei.pullFile(this.serial, remotePath, localPath);
    if (isXiaoweiFailure(res)) {
      throw new ADBError(res.message || 'pullFile failed', { serial: this.serial });
    }
    this.log.info('file_pulled', { remote: remotePath, local: localPath });
    return res;
  }
