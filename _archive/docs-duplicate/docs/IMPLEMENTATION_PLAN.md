...
**주의:** 현재(Phase 0) 스키마는 `channels.id`, `videos.id`가 **text** (YouTube ID)이며, 이 마이그레이션은 **UUID PK** 기준입니다. 기존 테이블이 이미 있으면 `CREATE TABLE` 시 충돌할 수 있으므로, 적용 전 다음 중 하나를 수행하세요: (1) `CREATE TABLE IF NOT EXISTS` 사용, (2) 기존 테이블 존재 시 `ALTER TABLE` 등으로 스키마 변경, (3) 새 DB/프로젝트에만 이 스크립트 적용. 자세한 현재 스키마는 `docs/docs/known-issues.md`의 Phase 0 / Phase 1 구분 참고.

```sql
-- 00002_channels_videos_schedules.sql

-- 채널 (기존 channels가 text PK면 이 CREATE 실패 → IF NOT EXISTS 또는 별도 마이그레이션)
...
CREATE TRIGGER schedules_updated_at BEFORE UPDATE ON schedules FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
```
