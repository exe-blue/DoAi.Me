...
async function main() {
  console.log("[Agent] Starting (PC number from local store or DB)");
  ...
  // 3. Resolve PC identity (from local store or register new via DB RPC)
  try {
    const pcNumber = await supabaseSync.getOrRegisterPcId();
    await supabaseSync.getPcId(pcNumber);
    console.log(`[Agent] PC ID: ${pcNumber}`);
  } catch (err) {
    console.error(`[Agent] ✗ PC resolution failed: ${err.message}`);
    process.exit(1);
  }
  ...
  broadcaster = new DashboardBroadcaster(supabaseSync.supabase, supabaseSync.pcNumber);
  try {
  ...
  deviceOrchestrator = new DeviceOrchestrator(xiaowei, supabaseSync.supabase, taskExecutor, {
    pcNumber: supabaseSync.pcNumber,
    pcUuid: supabaseSync.pcUuid,
    maxConcurrent: config.maxConcurrentTasks || 10,
    loggingDir: config.loggingDir,
  });
  console.log(`[Agent] DeviceOrchestrator pcNumber=${supabaseSync.pcNumber}`);
...
async function shutdown() {
  ...
  if (supabaseSync && supabaseSync.pcUuid) {
    try {
      await supabaseSync.updatePcStatus("offline");
