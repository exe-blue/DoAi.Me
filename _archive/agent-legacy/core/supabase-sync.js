...
const pcStore = require("./pc-store");
...
class SupabaseSync {
  constructor(supabaseUrl, supabaseAnonKey, supabaseServiceRoleKey) {
    ...
    /** @type {string|null} pc_number (e.g. "PC-01") */
    this.pcNumber = null;
    /** @type {string|null} pcs.id UUID */
    this.pcUuid = null;
  ...
  /**
   * Get or register PC identity: read from local store, or call RPC to register a new PC.
   * @returns {Promise<string>} pc_number (e.g. "PC-01")
   */
  async getOrRegisterPcId() {
    const saved = pcStore.getSavedPcNumber();
    if (saved) {
      return saved;
    }
    const { data: pcNumber, error } = await this.supabase.rpc("register_new_pc");
    if (error) {
      throw new Error(`Failed to register new PC: ${error.message}`);
    }
    if (!pcNumber || typeof pcNumber !== "string") {
      throw new Error("register_new_pc() did not return a pc_number");
    }
    pcStore.setSavedPcNumber(pcNumber);
    console.log(`[Supabase] Registered new PC: ${pcNumber}`);
    return pcNumber;
  }

  /**
   * Look up PC by pc_number and set this.pcNumber / this.pcUuid. Does not insert (use getOrRegisterPcId + RPC for new PCs).
   * @param {string} pcNumber - e.g. "PC-01"
   * @returns {Promise<string>} pc_number
   */
  async getPcId(pcNumber) {
    ...
    throw new Error(
      `PC not found: ${pcNumber}. Register first via getOrRegisterPcId() or ensure the row exists in pcs.`
    );
  }

  /**
   * Update PC heartbeat status (uses this.pcUuid).
   * @param {string} status - 'online' | 'offline' | 'error'
   */
  async updatePcStatus(status) {
    const update = {
      status,
  ...
  /**
   * Upsert device record by serial_number (uses this.pcUuid for pc_id).
   * @param {string} serial
   * @param {string} status
   * @param {string} model
   * @param {number|null} battery
   */
  async upsertDevice(serial, status, model, battery) {
    const { error } = await this.supabase
      .from("devices")
      .upsert(
        {
   ...
   * @param {string} [_pcNumber] - unused; pc_id is taken from this.pcUuid
   * @returns {Promise<Array>} upserted rows with id, serial_number
   */
  async batchUpsertDevices(devices, _pcNumber) {
  ...
  /**
   * Get task counts for this PC (uses this.pcUuid).
  ...
  /**
   * Get proxy counts for this PC (uses this.pcUuid).
  ...
  /**
   * Get device counts for this PC (uses this.pcUuid).
   ...
   * Phase 7: use mark_device_offline RPC so task_devices are rolled back (zombie cleanup). Uses this.pcUuid.
   ...
   */
  async markOfflineDevices(activeSerials, errorSerials = []) {
    const now = new Date().toISOString();
  ...
  /**
   * Get pending tasks assigned to this PC or unassigned (pc_id=null). Uses this.pcUuid.
   ...
   * @param {string} pcNumber - pc_number for log display (e.g. "PC-01")
   ...
   */
  subscribeToBroadcast(pcNumber, callback, timeoutMs = 10000) {
    console.log(`[Supabase] Subscribing to Broadcast room:tasks for PC ${pcNumber}`);
   ...
   * @returns {Promise<{status: string, channel: object}>}
   */
  subscribeToTasks(pcNumber, callback, timeoutMs = 10000) {
    console.log(`[Supabase] Subscribing to postgres_changes for PC ${pcNumber}`);
