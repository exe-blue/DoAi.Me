...
class TaskExecutor {
  ...
  async runTaskDevice(taskDevice) {
    ...
    // Use connection_id (IP:5555) for TCP devices; fallback to serial for USB (doc: device-layer-js-mapping)
    const connectionTarget = await this.supabaseSync.getDeviceTargetForTaskDevice(taskDevice) || serial;
    ...
    const engagementConfig = {
      ...
      channelName: "",
      videoId,
      warmupSec: this._shouldWarmup(connectionTarget) ? _randInt(60, 180) : 0,
      waitMinSec,
      waitMaxSec,
      commentContent,
  ...
  async execute(task) {
    ...
    try {
      ...
      await this.supabaseSync.insertExecutionLog(
        ...
        result,
        "success",
        `Task completed (${durationSec}s)${summarySuffix}`
      );
      ...
      console.log(`[TaskExecutor] ✓ ${task.id} completed (${durationSec}s)${summarySuffix}`);
    } catch (err) {
      const durationSec = ((Date.now() - startTime) / 1000).toFixed(1);
      this.stats.failed++;
