...
function startHeartbeat(xiaowei, supabaseSync, config, taskExecutor, broadcaster, reconnectManager, getDeviceOrchestrator) {
  const pcNumber = supabaseSync.pcNumber;
  ...
  console.log(
    `[Heartbeat] Starting (every ${interval / 1000}s) for PC ${pcNumber}`
  );
  ...
  async function beat() {
    try {
      ...
      await supabaseSync.updatePcStatus("online");

      // 3. Batch upsert all devices in a single query
      const activeSerials = devices.filter(d => d.serial).map(d => d.serial);
      const upsertedRows = await supabaseSync.batchUpsertDevices(devices, pcNumber);
      ...
      await supabaseSync.markOfflineDevices(activeSerials, errorSerials);

      // 7. Get aggregate counts for dashboard snapshot
      if (broadcaster) {
        try {
          const deviceCounts = await supabaseSync.getDeviceCounts();
          const taskCounts = await supabaseSync.getTaskCounts();
          const proxyCounts = await supabaseSync.getProxyCounts();
