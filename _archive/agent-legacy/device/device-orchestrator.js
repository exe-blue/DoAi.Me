...
class DeviceOrchestrator {
   ...
   * @param {object} config - { pcNumber (pc_number string), pcUuid (UUID), maxConcurrent? }
   */
  constructor(xiaowei, supabase, taskExecutor, config) {
    this.xiaowei = xiaowei;
    this.supabase = supabase;
    this.taskExecutor = taskExecutor;
    this.pcNumber = config.pcNumber;
  ...
  start() {
    ...
    console.log(`[DeviceOrchestrator] Starting pcNumber=${this.pcNumber} (every ${ORCHESTRATE_INTERVAL_MS / 1000}s, maxConcurrent=${this.maxConcurrent})`);
  ...
  async _assignWork(serial) {
    ...
    if (process.env.DEBUG_ORCHESTRATOR || process.env.DEBUG_ORCHESTRATOR_CLAIM) {
      console.log(`[DeviceOrchestrator] _assignWork(${serial.substring(0, 6)}) pcNumber=${this.pcNumber}`);
    }
      ...
      const attempts = [
        {
          ...
          params: { runner_pc_name: this.pcNumber, max_to_claim: 1 },
        },
        this.pcUuid
          ? {
            rpc: "claim_task_devices_for_pc",
