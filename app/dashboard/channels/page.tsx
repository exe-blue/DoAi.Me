...
export default function ChannelsPage() {
  ...
  const [videosCache, setVideosCache] = useState<Record<string, ChannelVideo[]>>({});

  const { data, error, isLoading, mutate } = useSWR<{
    channels: Channel[];
    contents?: unknown[];
  }>(CHANNELS_KEY, fetcher, { refreshInterval: 60_000 });
  ...
  const toggleExpand = async (channelId: string) => {
    ...
    if (!videosCache[channelId]) {
      const res = await apiClient.get<{ videos: ChannelVideo[] }>(
        `/api/channels/${channelId}/videos`
      );
      const list =
        res.success && res.data?.videos ? res.data.videos : [];
      setVideosCache((prev) => ({ ...prev, [channelId]: list }));
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {channels.length}개 등록 · {monitored.length}개 모니터링
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {channels.map((ch) => {
            ...
            const chVideos = videosCache[ch.id] ?? [];
            return (
              <div
                key={ch.id}
              ...
              >
                ...
                >
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#1a1d2e]">
                    {ch.thumbnail_url ? (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {ch.is_monitored && (
                        <span className="rounded bg-green-900/30 px-1.5 py-0.5 text-[8px] font-bold text-green-400">
                          모니터링
                        </span>
                      )}
                      {ch.auto_collect && (
                    ...
                    <div className="mt-1 flex items-center gap-3 text-xs text-slate-500">
                      ...
                      <span className="flex items-center gap-0.5">
                        ...
                        동기화 {timeSince(ch.last_collected_at)}
                      </span>
                    </div>
                  </div>
                  <div
                  ...
                  >
                    <button
                      ...
                      className="rounded p-1.5 text-slate-600 hover:bg-red-900/10 hover:text-red-400"
                      title="삭제"
                      aria-label="삭제"
                    >
                      ...
                      <div className="max-h-60 space-y-1.5 overflow-y-auto">
                        {chVideos.map((v) => {
                          const pct =
                            v.target_views && v.target_views > 0
                              ? Math.min(
                                  100,
                                  Math.round(
                                    ((v.completed_views ?? 0) / v.target_views) *
                                      100
                                  )
                                )
                              : 0;
                          return (
                            <div
                              key={v.id}
                              className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-[#12141d]/50"
                            >
                              <div
                                className={cn(
                              ...
                              {v.duration_sec != null && (
                                <span className="font-mono text-[10px] text-slate-500">
                                  {Math.round(v.duration_sec / 60)}분
                                </span>
                              )}
                              {v.target_views != null && v.target_views > 0 && (
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="border-[#1e2130] bg-[#0f1117] text-slate-200 sm:max-w-md">
          ...
          <p className="text-xs text-slate-500">
            YouTube 채널 URL 또는 핸들을 입력하세요 (예: @handle)
          </p>
          <Input
            value={addUrl}
