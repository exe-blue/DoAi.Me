...
```
신규 생성 → 워밍업 (3~7일) → 활성 (미션 투입) → 쿨다운 → 활성 → ... → 밴
```

| 단계 | 기간 | 활동 |
|------|------|------|
| **워밍업** | 3~7일 | 랜덤 시청만 (좋아요/댓글 금지), 1일 5~10회 |
| **활성** | 지속 | 미션 수행, 로테이션 적용 |
| **쿨다운** | 30분~2시간 | 활동 중지 (과도한 활동 후 자동 진입) |
| **밴** | 영구 | 풀에서 제거, 보충 필요 |
...
| 일일 로테이션 | **기기당 3~4개 계정** | 4시간 × 3 = 12시간 커버 |
| 동시 같은 채널 | **같은 계정으로 같은 채널 2회 이상 금지** | 패턴 감지 방지 |

### 1.3 계정 풀 규모

```
기기 500대 × 로테이션 3개/일 = 1,500 활성 계정 필요
+ 워밍업 중: ~200개 (신규 투입 대기)
+ 쿨다운 여유: ~300개
+ 밴 예비: ~200개
───────────────────
...
예상 월간 밴률: 5~10% = 110~220개/월
→ 월 200개 이상 보충 파이프라인 필요
```
...
| WiFi vs LTE | WiFi만 | 가능하면 일부 LTE (SIM 필요) |
