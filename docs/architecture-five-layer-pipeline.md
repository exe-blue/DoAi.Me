...
**Agent 코드 경로:** 실행 컨텍스트는 `apps/desktop/src/agent` (Dev), 패키징 시 `resources/agent`. 아래 "구현: agent/..."는 위 경로 내 상대 경로(예: device/heartbeat.js, core/supabase-sync.js)로 해석.
...
- **구현**: device/heartbeat.js, core/supabase-sync.js, agent.js 3(Register PC), 8(Start heartbeat). (실행: apps/desktop/src/agent 또는 resources/agent)
...
- **요구**: (1) task_devices 수만큼 PC가 푸시 수신 (2) 시청시간·액션 확률로 config 구체화 (3) 터치좌표 포함 (4) 댓글 수만큼 AI 사전 생성(Edge Function) (5) 사전데이터 서버 전송 후 로컬 스케줄러 배정 (6) xiaowei 명령.
- **구현**: config는 서버(트리거)에서 채움. 댓글은 Edge Function generate-comments(제목·본문→N개)로 생성 후 task.payload.comments→트리거가 config.comment_pool로 넣음. device-orchestrator가 claim → runTaskDevice 호출로 “작업 배정”.

---

## 5. 디바이스 실행 레이어 (명령 실행, 로그 송신)

- **구현**: task/task-executor.js runTaskDevice, _watchVideoOnDevice, insertExecutionLog. device-orchestrator claim → complete_task_device / fail_or_retry_task_device. (실행: apps/desktop/src/agent 또는 resources/agent)
