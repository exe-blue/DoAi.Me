...
export class Broadcaster {
  ...
  async broadcastWorkerDevices(devices: XiaoweiDevice[]): Promise<void> {
    const topic = `room:worker:${this.workerId}:devices`;
  ...
  async broadcastWorkerHeartbeat(
    deviceCount: number,
    xiaoweiConnected: boolean
  ): Promise<void> {
    const topic = `room:worker:${this.workerId}`;
  ...
  private async sendBroadcast(
    ...
    try {
      ...
      if (!channel) {
        ...
        // Register a no-op handler to enable broadcast mode before subscribing
        channel.on("broadcast", { event: "__noop__" }, () => {});
        await new Promise<void>((resolve, reject) => {
          channel!.subscribe((status) => {
            if (status === "SUBSCRIBED") resolve();
            else if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
              reject(new Error(`Channel subscribe failed: ${topic} (${status})`));
            }
          });
        });
