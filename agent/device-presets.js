...
async function optimize(xiaowei, serial, options = {}) {
  ...
  const cmds = [
    ...
    // 해상도 (물리 해상도 사용, override 제거)
    { cmd: "wm size reset", desc: "해상도 원본 복원" },
    { cmd: "wm density reset", desc: "밀도 원본 복원" },
...
async function ytTest(xiaowei, serial, options = {}) {
  ...
  try {
    // 1. YouTube 종료 + 실행
    await xiaowei.stopApk(serial, "com.google.android.youtube");
    await _sleep(1000);
    await xiaowei.startApk(serial, "com.google.android.youtube");
    ...
    if (sm) {
      ...
      await xiaowei.tap(serial, cx, cy);
      step("검색 버튼 터치", true, `(${cx},${cy})`);
    } else {
      // 폴백: 우측 상단
      await xiaowei.tap(serial, 930, 80);
    ...
    if (videoMatch) {
      ...
      await xiaowei.tap(serial, 540, 770);
...
async function warmup(xiaowei, serial, options = {}) {
  ...
  try {
    ...
    while (Date.now() - startTime < targetMs && result.videosWatched < 5) {
      ...
      await xiaowei.tap(serial, midX, tapY);
      await _sleep(_randInt(3000, 5000));
  ...
  const apks = [
    {
      ...
      path:
        ...
        "C:\\Program Files (x86)\\xiaowei\\tools\\XWKeyboard.apk",
    ...
    },
    {
      ...
      path:
        ...
        "C:\\Program Files (x86)\\xiaowei\\tools\\assistant.apk",
      package: null,
    },
    {
      name: "HID Manager",
      path:
        ...
        "C:\\Program Files (x86)\\xiaowei\\tools\\hidmanager.apk",
      package: null,
    },
  ];
